...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope("recurrent_layer"):
            ...
            self.out_, self.final_h_ = tf.nn.dynamic_rnn(self.cell_, self.x_, dtype=tf.float32)
        ...
        with tf.name_scope("output_layer"):
            self.W_out_ = tf.Variable(tf.random_uniform([self.H, self.V]), name="W_out")
    ...
    def BuildTrainGraph(self):
        ...
        with tf.name_scope("training"):
            ...
            self.out_2d_ = tf.reshape(self.out_, [-1, self.H])
            self.target_y_2d_ = tf.reshape(self.target_y_, [-1, 1])

            per_example_train_loss_ = tf.nn.sampled_softmax_loss(weights=tf.transpose(self.W_out_),
                                                                 biases=self.b_out_,
                                                                 labels=self.target_y_2d_,
                                                                 inputs=self.out_2d_,
    ...
    def BuildSamplerGraph(self):
        ...
        with tf.name_scope("sampler_graph"):
            logits2_ = tf.reshape(self.logits_, [self.batch_size_ * self.max_time_, self.V])
            self.pred_samples_orig_ = tf.multinomial(logits2_, num_samples=1, name="pred_random")
            self.pred_samples_ = tf.reshape(self.pred_samples_orig_, [self.batch_size_, self.max_time_, 1])
        #### YOUR CODE HERE ####
