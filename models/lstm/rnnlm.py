...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope("Embedding_Layer"):
            # embedding matrix (vocab length, embedding length)
            self.W_in_ = tf.Variable(tf.random_uniform([self.V, self.H], -1.0, 1.0), name="W_in")
            # embedding_lookup gives shape (batch_size, max seq length, embedding length)
            self.input_w_emb_ = tf.reshape(tf.nn.embedding_lookup(self.W_in_, self.input_w_), 
                                           [self.batch_size_, self.max_time_, self.H], name="input_w_emb")

        # Construct RNN/LSTM cell and recurrent layer.
        with tf.name_scope("Recurrent_Layer"):
            # Construct RNN/LSTM cell
            self.cell_ = MakeFancyRNNCell(self.H, self.dropout_keep_prob_, self.num_layers)
            # Contruct h init from cell 
            self.initial_h_ = self.cell_.zero_state(self.batch_size_, dtype=tf.float32)
            # Creates a recurrent neural network specified by cell_
            # Performs fully dynamic unrolling of inputs input_w_emb_
            # It returns the output layer from each cell through the sequence, as well as the final hidden state
            self.outputs_, self.final_h_ = tf.nn.dynamic_rnn(self.cell_, self.input_w_emb_, sequence_length=self.ns_, 
        ...
        with tf.name_scope("Output_Layer"):
            ...
            # Compute logits. [batch size, max time, H size] x [H, V size] = [batch size, max time , V size]
            self.logits_ = matmul3d(self.outputs_, self.W_out_) + self.b_out_
        
        # Loss computation (true loss, for prediction)
        # Reduce mean of per example loss
        with tf.name_scope("Softmax"):      
            self.loss_ = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.target_y_, logits=self.logits_, name="loss")) 
    ...
    def BuildTrainGraph(self):
        ...
        with tf.name_scope("Training"):
            ...
            # Wout [H, V]; bout [V,]; targetY [batch, max time]; outputs [batch, max time, H]
            
            # Arg dimensions for sampled_softmax_loss function: 
            # weights [num_classes, dim]; biases [num_classes]; labels [batch_size, num_true]; inputs [batch_size, dim]
            
            # After reshape, we input the following into the args:
            # Wout [V, H]; bout [V,]; targetY [batch * max time, 1]; outputs [batch * max time, H]
            
            # the sampled softmax loss function does not accept the three dimensional batch x sequence x final_hidden_layer shape, so
            # you'll need to reshape both the tensor holding the output hidden layers and the labels so that you are making only one
            # prediction per row (hint, you'll need batch x sequence rows)
            per_example_train_loss_ = tf.nn.sampled_softmax_loss(weights=tf.transpose(self.W_out_), 
                                                                 biases=self.b_out_, 
        ...
        # Provide train_loss_ which is the tensor containing the value to minimize.
        # train_step_ is an operation that updates the variables. If global_step was not None, that operation also increments global_step
        
        # Adam Optimizer (ideal learning rate of 0.01)
...
#         # Adagrad Optimizer (ideal learning rate of 0.1)
#         self.optimizer_ = tf.train.AdagradOptimizer(learning_rate=self.learning_rate_)
#         self.train_step_ = self.optimizer_.minimize(self.train_loss_)
    ...
    def BuildSamplerGraph(self):
        ...
        # reshape logits [batch size, max time , V size] to [batch * max time,  V] then extract 1 sample
        pred_samples_pre_ = tf.multinomial(tf.reshape(self.logits_, [-1, self.V]), num_samples=1)
        
        # reshape back to [batch size, max time , 1]
        self.pred_samples_ = tf.reshape(pred_samples_pre_, [self.batch_size_, self.max_time_, 1])
